...
public interface FollowRepository extends JpaRepository<Follow, Long> {
  ...
  @Query("select u from users u left join follows f on f.followerId = ?1 where f.followingId = ?1")
  List<User> findAllFollowerUserByUsername(User user);


  @Query("select u from users u left join follows f on f.followingId = ?1 where f.followerId = ?1")
